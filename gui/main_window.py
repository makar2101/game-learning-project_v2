"""
Main Window - оновлена версія тільки з граматичними поясненнями
Спрощений інтерфейс без перекладів та згортання
"""
        ...
        def create_next_batch():
            ...
            for i in range(current_index, batch_end):
                ...
                try:
                    ...
                    # Створюємо спрощений SentenceWidget
...
🤖 ОСНОВНА ІНФОРМАЦІЯ:
• Доступність: {'✅ Доступний' if status['available'] else '❌ Недоступний'}
• Модель: {status.get('model', 'Невідома')}
• Клієнт: {status.get('client', 'Невідомий')}

📊 СТАТИСТИКА ВИКОРИСТАННЯ:
• Граматичні пояснення: {status['usage_stats'].get('grammar_explanations', 0)}
• Загальна кількість запитів: {sum(status['usage_stats'].values()) - (1 if 'last_request' in status['usage_stats'] else 0)}
• Помилки: {status['usage_stats'].get('errors', 0)}
• Останній запит: {status['usage_stats'].get('last_request', 'Немає')}

⚙️ КОНФІГУРАЦІЯ:
• База URL: {self.ai_manager.get_config().get('ollama', {}).get('base_url', 'Невідома')}
• Температура: {self.ai_manager.get_config().get('ollama', {}).get('temperature', 'Невідома')}
• Таймаут: {self.ai_manager.get_config().get('ollama', {}).get('timeout', 'Невідомий')} сек
"""

            # Створюємо вікно статусу
            status_window = tk.Toplevel(self.root)
            status_window.title("Статус AI")
            status_window.geometry("450x350")
            status_window.resizable(True, True)

            # Текстове поле зі статусом
            ...
            messagebox.showerror("Помилка", f"Не вдалося показати статус AI: {e}")

    def refresh_ai_status(self, text_widget):
        """Оновлює статус AI в вікні"""
        try:
            status = self.ai_manager.get_status()

            status_text = f"""Статус AI системи (оновлено):
...
"""
            ...
            self.logger.error(f"Помилка оновлення статусу AI: {e}")

    def test_ai_connection(self, text_widget):
        """Тестує підключення до AI"""
        try:
            result = self.ai_manager.test_ai()

            if result['success']:
                test_result = f"\n\n✅ ТЕСТ ПРОЙШОВ УСПІШНО:\n{result['message']}"
                if 'test_translation' in result:
                    test_result += f"\nТестовий переклад: {result['test_translation']}"
            else:
                test_result = f"\n\n❌ ТЕСТ НЕ ПРОЙШОВ:\n{result['message']}"

            text_widget.config(state=tk.NORMAL)
            text_widget.insert(tk.END, test_result)
            text_widget.config(state=tk.DISABLED)
            text_widget.see(tk.END)

        except Exception as e:
            self.logger.error(f"Помилка тестування AI: {e}")
            test_result = f"\n\n❌ ПОМИЛКА ТЕСТУВАННЯ:\n{str(e)}"
            text_widget.config(state=tk.NORMAL)
            text_widget.insert(tk.END, test_result)
            text_widget.config(state=tk.DISABLED)
